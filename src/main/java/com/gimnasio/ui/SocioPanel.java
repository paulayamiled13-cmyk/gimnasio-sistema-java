...
public class SocioPanel extends JPanel {
    ...
    public SocioPanel(Usuario usuario) {
    ...
    private void build() {
        ...
        table.setModel(new DefaultTableModel(
                new String[] { "ID", "Nombres", "Apellidos", "DNI", "Teléfono", "Correo", "Dirección", "Estado" }, 0));
        ...
        JPanel panelTabla = crearPanelTabla("Listado de socios registrados", scrollTabla);
    ...
    private JPanel crearFormulario() {
        ...
        JLabel titulo = new JLabel("Datos del socio");
        titulo.setFont(new Font("Segoe UI", Font.BOLD, 18));
        titulo.setForeground(new Color(25, 25, 25));
        titulo.setBorder(BorderFactory.createEmptyBorder(0, 0, 8, 0));

        JPanel form = new JPanel(new GridLayout(4, 4, 10, 7));
        form.setOpaque(false);

        estilizarCampo(txtNombres);
        ...
        form.add(label("Nombres"));
        form.add(txtNombres);
        form.add(label("Apellidos"));
        form.add(txtApellidos);

        form.add(label("DNI"));
        form.add(txtDni);
        form.add(label("Teléfono"));
        form.add(txtTelefono);

        form.add(label("Correo"));
        form.add(txtCorreo);
        form.add(label("Dirección"));
    ...
    private void cargar() {
        try {
            ...
            for (Socio s : dao.listar(txtBuscar.getText().trim())) {
    ...
    private void seleccionar() {
        ...
        txtNombres.setText(table.getValueAt(row, 1).toString());
        txtApellidos.setText(table.getValueAt(row, 2).toString());
        txtDni.setText(table.getValueAt(row, 3).toString());
        txtTelefono.setText(table.getValueAt(row, 4).toString());
        txtCorreo.setText(table.getValueAt(row, 5).toString());
        txtDireccion.setText(table.getValueAt(row, 6).toString());
        cmbEstado.setSelectedItem(table.getValueAt(row, 7).toString());
    }
    ...
    private void guardar() {
        try {
            ...
            SwingUtil.info(this, "Registro guardado correctamente.");
            limpiar();
            cargar();
    ...
    private void baja() {
        try {
            if (txtId.getText().isEmpty()) {
                SwingUtil.error(this, "Seleccione un socio de la tabla para darlo de baja.");
                return;
            }

            if (SwingUtil.confirm(this, "¿Desea dar de baja al socio seleccionado?")) {
                dao.darBaja(Integer.parseInt(txtId.getText()));
                cargar();
                limpiar();
            }
